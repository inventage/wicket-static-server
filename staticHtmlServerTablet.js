...
const WICKET_EXTEND_ROOT = path.resolve(`${__dirname}/../../../../../`);
...
/**
 * It seems, that the connect-livereload is not working as expected. we will fix it with this.
 *
 * @param html
 * @returns {*}
 */
...
function expandPages(html, variables, additionalHeadElements) {
    ...
    const parentPage = readWicketHtmlFile(found[1]);
    if (!parentPage) {
        throw new Error(`No parent page found at… ${html.substring(0, 50)}`);
    }

    parseExtendVariables(variables, found[2]);
    const childPage = found[3];
...
/**
 * All tablet pages listing
 */
app.get('/pages', (req, res) => {
    staticPageListing(res, /tablet\/.*Page\.html/, 'pages', 'Tablet Pages');
});
